...
export default function TrustNetworkManager({ memberCode }: TrustNetworkManagerProps) {
  ...
  const loadTrustData = async () => {
    ...
    try {
      ...
      // Load trust bonds
      const bondsResponse = await fetch(`/api/trust-bonds/list?memberCode=${memberCode}`);
      ...
      const memberResponse = await fetch(`/api/user/profile?memberCode=${memberCode}`);
  ...
  const handleMemberClick = async (memberCode: string, memberName: string, isSponsor: boolean = false) => {
    try {
      ...
      <div className="bg-white rounded-lg shadow-sm border border-gray-200 p-6">
        ...
        {trustBonds.length === 0 ? (
          <div className="text-center py-8 text-gray-500">
            ...
            <p className="text-sm mt-2">Create connections to build your trust network!</p>
          </div>
        ) : (
              ...
              >
                <div className="flex items-center space-x-3">
                  ...
                  <div>
                    <p className="font-medium text-gray-800">
                      {bond.fromMemberCode === memberCode ? bond.toMemberName : bond.fromMemberName}
                    </p>
                    <p className="text-xs text-gray-500">
                      {bond.fromMemberCode === memberCode ? bond.toMemberCode : bond.fromMemberCode}
                    </p>
                  </div>
                </div>
                ...
                <div className="flex items-center space-x-2">
                  <span className={`px-2 py-1 rounded-full text-xs font-medium ${
                    ...
                    {bond.status === 'accepted' ? '✅ Connected' : 
                     bond.status === 'pending' ? '⏳ Pending' : 
                     '❌ Rejected'}
                  </span>
                  
                  {bond.status === 'accepted' && (
                    <span className="text-xs text-gray-500">
                      {bond.createdAt ? new Date(bond.createdAt._seconds ? bond.createdAt._seconds * 1000 : bond.createdAt).toLocaleDateString() : 'N/A'}
                    </span>
                  )}
                </div>
              </div>
      ...
      <div className="bg-white rounded-lg shadow-sm border border-gray-200 p-6">
          ...
          <div className="mb-6">
            ...
            {trustMembers.length === 0 ? (
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
                ...
                {trustStatus?.trustUnit?.sponsorCode && trustStatus.trustUnit.sponsorCode !== '0000000000' && (
                      ...
                      <div className="w-12 h-12 rounded-full bg-gradient-to-br from-blue-500 to-purple-600 flex items-center justify-center text-white font-bold text-lg overflow-hidden">
                        {trustStatus.trustUnit.sponsorProfilePicture ? (
                          <img 
                            ...
                            alt={trustStatus.trustUnit.sponsorName || 'Sponsor'}
                        ...
                        <div className={`w-full h-full flex items-center justify-center ${trustStatus.trustUnit.sponsorProfilePicture ? 'hidden' : 'flex'}`}>
                          👑
                        </div>
                      </div>
                      <div className="flex-1">
                        <div className="flex items-center space-x-2">
                          <h3 className="font-semibold text-gray-800">{trustStatus.trustUnit.sponsorName || 'Sponsor'}</h3>
                          <button
                            onClick={() => handleMemberClick(trustStatus.trustUnit.sponsorCode, trustStatus.trustUnit.sponsorName, true)}
                ...
                {trustMembers.filter(member => member.memberCode !== trustStatus?.trustUnit?.sponsorCode).map((member) => (
                  <div
                    ...
                    <div className="flex items-center space-x-3">
                      ...
                      <div className="flex-1">
                        <div className="flex items-center space-x-2">
                          ...
                          <button
                            onClick={() => handleMemberClick(member.memberCode, member.name, false)}
        ...
        {!trustStatus?.trustUnit && (
          <div className="text-center py-8 text-gray-500">
            ...
            <p className="text-sm mt-2">Create trust bonds to start building your network!</p>
          </div>
        )}
      </div>
              ...
              <div className="flex flex-col items-center space-y-4">
                ...
                <div className="w-20 h-20 rounded-full bg-gradient-to-br from-blue-500 to-purple-600 flex items-center justify-center text-white font-bold text-2xl">
                  {selectedMember.profilePicture ? (
                    ...
                    (selectedMember.displayName || selectedMember.name || 'M').charAt(0).toUpperCase()
                  )}
                </div>
                ...
                <div className="text-center">
                  <h4 className="text-xl font-semibold text-gray-800">
                    {selectedMember.displayName || selectedMember.name || 'Unknown Member'}
                  </h4>
                  {selectedMember.isSponsor && (
                ...
                <div className="text-center">
                  ...
                  <p className="text-lg font-mono text-gray-800">{selectedMember.memberCode}</p>
                </div>
                ...
                <div className="w-full space-y-3">
                  ...
                  {/* Sponsor */}
                  {selectedMember.sponsorName && selectedMember.sponsorId !== '0000000000' && (
                    <div className="flex items-center space-x-3 p-3 bg-gray-50 rounded-lg">
                      ...
                      <div>
                        ...
                        <p className="text-sm font-medium text-gray-800">{selectedMember.sponsorName}</p>
                        <p className="text-xs text-gray-500">{selectedMember.sponsorId}</p>
                      </div>
                    </div>
                  )}
                </div>
