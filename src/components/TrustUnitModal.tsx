...
export default function TrustUnitModal({
  ...
  const currentMember = trustUnit.members.find(
    m => m.memberCode === currentMemberCode
  );
  // ✅ DEDUPLICATE members to prevent React key errors
  const allMembers = trustUnit.members.filter(
    (member, index, self) =>
        ...
        <div className='px-6 py-4'>
              ...
              return (
                ...
                >
                  <div className='flex items-center space-x-3'>
                    ...
                    <div>
                      <p
                        className={`font-medium ${isCurrentUser ? 'text-slate-900' : 'text-slate-800'}`}
                      >
                        {member.name}
                      </p>
                      <p
                        className={`text-xs ${isCurrentUser ? 'text-blue-600 font-medium' : 'text-slate-500'}`}
                      >
                        {isCurrentUser
                          ? 'You'
          ...
          <div className='mt-4 p-3 bg-amber-50 rounded-lg'>
            <p className='text-sm text-amber-800'>
              <strong>Instructions:</strong> Click "Connect" to join the Trust
              Unit, or "Wait" to defer your decision. All members must connect
              for the Trust Unit to be fully established.
            </p>
          </div>
        </div>
