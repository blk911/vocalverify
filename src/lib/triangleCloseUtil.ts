...
import { logTriangleCloseDetected, logTUCreation, logTUUpdate } from './telemetry';
...
export async function detectTriangleClose(
  ...
  try {
    ...
    console.log(`📊 [TRIANGLE-CLOSE] Inviter: root=${inviterRoot}, depth=${inviterDepth}`);
    console.log(`📊 [TRIANGLE-CLOSE] Invitee: root=${inviteeRoot}, depth=${inviteeDepth}`);
    ...
    if (inviterDepth < 1 || inviteeDepth < 1) {
      ...
      return { shouldCreateTU: false, reason: 'Cannot form TU with root members' };
    }

    // Rule 3: Check if they're already in a TU together with this root
    console.log(`🔍 [TRIANGLE-CLOSE] Checking for existing TU with root ${inviterRoot}`);
    
    const existingTUQuery = await db.collection('trustUnits')
    ...
    logTriangleCloseDetected(inviterCode, inviteeCode, inviterRoot, 'tu_created');
...
export async function createOrUpdateTriangleTU(
  ...
  inviteeCode: string,
  rootSponsorId: string
): Promise<{ success: boolean; unitId: string | null; action: 'created' | 'updated' | 'none' }> {
  ...
  try {
    ...
    if (!existingTUQuery.empty) {
      ...
      if (membersToAdd.length > 0) {
        ...
        console.log(`✅ [TU-CREATE] Updated TU ${tuDoc.id} with ${membersToAdd.length} new members`);
