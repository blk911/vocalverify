...
export function getFeatureFlags(): FeatureFlags {
  ...
  return {
    ...
    triangleCloseTUs: tuTriangleEnabled && (process.env.NEXT_PUBLIC_TRIANGLE_CLOSE_TU !== 'false'),
    ...
    tuInviteBlocker: tuTriangleEnabled && (process.env.NEXT_PUBLIC_TU_INVITE_BLOCKER !== 'false'),
    
    // ℹ️  Detect circular invites (log only, doesn't block)
    circularInviteDetection: tuTriangleEnabled && (process.env.NEXT_PUBLIC_CIRCULAR_INVITE_DETECTION !== 'false'),
  };
}
...
export function logFeatureFlags(): void {
  ...
  console.log(`   - Triangle Close TUs: ${flags.triangleCloseTUs ? '✅ ENABLED' : '❌ DISABLED'}`);
  console.log(`   - Same-Sponsor TUs: ${flags.sameSponsorTUs ? '✅ ENABLED' : '❌ DISABLED'}`);
  console.log(`   - TU Invite Blocker: ${flags.tuInviteBlocker ? '✅ ENABLED' : '❌ DISABLED'}`);
  console.log(`   - Circular Invite Detection: ${flags.circularInviteDetection ? '✅ ENABLED' : '❌ DISABLED'}\n`);
}
