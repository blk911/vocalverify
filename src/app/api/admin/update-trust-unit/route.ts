...
export async function PUT(request: NextRequest) {
  try {
    ...
    if (!unitId || !memberCodes || !Array.isArray(memberCodes)) {
      return NextResponse.json({ error: 'Unit ID and member codes are required' }, { status: 400 });
    }
    ...
    if (!tuDoc.exists) {
      return NextResponse.json({ error: 'Trust Unit not found' }, { status: 404 });
    }
    ...
    console.log(`✅ Updated Trust Unit ${unitId} with ${members.length} members`);
