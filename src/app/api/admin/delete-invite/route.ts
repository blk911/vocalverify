...
export async function DELETE(req: NextRequest) {
  try {
    ...
    console.log('[DELETE-INVITE] Request received:', { inviteId, memberCode, phone });
    ...
    if (inviteDoc.exists) {
      ...
      console.log('[DELETE-INVITE] Invite not found in invites collection:', inviteId);
    }
    ...
    if (phone) {
      ...
      usersByPhone.docs.forEach(doc => {
        ...
        console.log('[DELETE-INVITE] Queued user by phone for deletion:', doc.id);
      });
      
      // Search temp users by phone
      const tempUsersByPhone = await db.collection('tempUsers')
        .where('phone', '==', phone)
        .limit(5)
        .get();
    ...
    if (phone) {
      const invitesByPhone = await db.collection('invites')
        .where('phone', '==', phone)
        .limit(10)
        .get();
      
      invitesByPhone.docs.forEach(doc => {
        if (doc.id !== inviteId) { // Don't double-delete
          batch.delete(doc.ref);
          deletedItems.push(`invite-by-phone:${doc.id}`);
          console.log('[DELETE-INVITE] Queued related invite for deletion:', doc.id);
        }
      });
    }
    ...
    await batch.commit();
    console.log('[DELETE-INVITE] Batch committed successfully. Deleted:', deletedItems);
