...
import { initializeDatabase, createSampleData, verifyDatabaseStructure } from "@/lib/databaseInit";
...
export const POST = asyncHandler(async (req: Request) => {
  ...
  try {
    ...
    if (verification.status !== 'healthy') {
      ...
      if (!sampleResult) {
        logger.warn('Sample data creation failed, but database is initialized', 'DatabaseInit');
      }
    }
