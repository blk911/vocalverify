  ...
  try {
    ...
    for (const collectionName of collections) {
      ...
      for (const doc of snapshot.docs) {
        ...
        console.log(
          `[MIGRATE-NAMES] ${collectionName}/${doc.id}: "${data.name}" -> nameLower: "${nameLower}"`
        );
        ...
        if (batchCount >= 500) {
          await batch.commit();
          console.log(
            `[MIGRATE-NAMES] Committed batch of ${batchCount} updates`
          );
          batchCount = 0;
        }
      }
      ...
      if (batchCount > 0) {
        await batch.commit();
        console.log(
          `[MIGRATE-NAMES] Committed final batch of ${batchCount} updates`
        );
      }
      ...
      console.log(
        `[MIGRATE-NAMES] ${collectionName} complete: ${updated} updated, ${skipped} skipped`
      );
    }
