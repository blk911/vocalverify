  ...
  try {
    ...
    if (!fromMemberCode || !toMemberCode) {
      return NextResponse.json(
        { ok: false, error: "Missing required fields: fromMemberCode, toMemberCode" },
        { status: 400 }
      );
    }
    ...
    const existingBond = await db.collection('trustBonds')
    ...
    return NextResponse.json(
      { ok: false, error: "Failed to create trust bond", details: error.message },
      { status: 500 }
    );
  }
}
