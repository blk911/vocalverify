...
export async function GET(req: NextRequest) {
  try {
    ...
    const connectionsQuery2 = await db
      .collection('trustConnections')
    ...
    const pendingBondsQuery = await db
    ...
    const outgoingBondsQuery = await db
      .collection('trustBonds')
    ...
    if (
      trustUnitData.sponsorCode &&
      trustUnitData.sponsorCode !== '0000000000'
    ) {
      try {
        const sponsorDoc = await db
          .collection('users')
          .doc(trustUnitData.sponsorCode)
          .get();
    ...
    return NextResponse.json(
      {
        ok: false,
        error: 'Failed to get trust unit status',
