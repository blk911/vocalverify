...
export async function GET(req: NextRequest) {
  try {
    ...
    const memberDetails = await Promise.all(
      memberCodes.map(async (code: string) => {
        try {
          ...
          logger.error(
            `Error fetching member details for ${code}`,
            error as Error,
            'TrustUnits'
          );
          return {
    ...
    logger.info(
      `Trust unit members retrieved for ${memberCode}`,
      'TrustUnits',
      {
    ...
    return NextResponse.json({
      ...
      count: memberDetails.length,
      directConnections: memberDetails.filter(
        m => m.connectionStatus === 'direct'
      ).length,
      indirectConnections: memberDetails.filter(
        m => m.connectionStatus === 'indirect'
      ).length,
    });
  } catch (error: any) {
    ...
    return NextResponse.json(
      {
        ok: false,
        error: 'Failed to get trust unit members',
