...
export async function GET(req: NextRequest) {
  ...
  try {
    ...
    if (!usersSnapshot.empty) {
      ...
      logger.info(`User found: ${name}`, 'UserCheck', {
    ...
    if (!invitesSnapshot.empty) {
      ...
      logger.info(`Pending invite found for: ${name}`, 'UserCheck', {
      ...
      console.log(
        '[CHECK-WITH-INVITE] Is admin invite?',
        inviteData.sponsorId === '0000000000'
      );
    ...
    logger.error('Database error in check-with-invite', error, 'UserCheck', {
      name,
    });
    throw error;
  }
