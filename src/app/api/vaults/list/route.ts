...
export async function GET(request: NextRequest) {
  try {
    ...
    const vaultsSnapshot = await db.collection('vaults')
    ...
    for (const doc of vaultsSnapshot.docs) {
      try {
        ...
        if (vaultData && vaultData.participants && Array.isArray(vaultData.participants)) {
          for (const participantCode of vaultData.participants) {
            if (participantCode !== memberCode) {
              try {
                const memberDoc = await db.collection('users').doc(participantCode).get();
                if (memberDoc.exists) {
                  const memberData = memberDoc.data();
                  participantDetails.push({
                    memberCode: participantCode,
                    name: memberData?.name || memberData?.displayName || 'Unknown',
                    profilePicture: memberData?.profilePicture || null
                  });
                } else {
                  // Handle orphaned participant (user deleted but vault remains)
                  console.log(`Orphaned participant ${participantCode} in vault ${doc.id}`);
                  participantDetails.push({
                    memberCode: participantCode,
                    name: 'Deleted User',
                    profilePicture: null
                ...
                console.error(`Error fetching member ${participantCode}:`, error);
              }
            }
          }
        }
