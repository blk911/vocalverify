...
export async function POST(request: NextRequest) {
  try {
    ...
    console.log('[INVITES-SEND] Request:', {
      memberCode,
      invitedName,
      invitedPhone,
    });
    ...
    console.log('[INVITES-SEND] Name normalized:', {
      input: invitedName,
      properName,
      nameLower,
    });
    ...
    if (flags.tuInviteBlocker) {
      console.log(
        '[INVITES-SEND] Checking for duplicate TU membership (ENABLED)...'
      );
      const blockerResult = await checkInviteBlock(
        memberCode,
        properName,
        invitedPhone
      );
      ...
      console.log(
        '[INVITES-SEND] TU invite blocker (DISABLED by feature flag)'
      );
    }

    // ✅ PHASE 4: Check for circular invite (allowed, but log it)
    if (flags.circularInviteDetection) {
      const circularCheck = await detectCircularInvite(
        memberCode,
        invitedPhone
      );
      if (circularCheck.isCircular) {
        console.log(
          `📍 [INVITES-SEND] Circular invite detected (allowed): ${circularCheck.reason}`
        );
    ...
    console.log(
      '[INVITES-SEND] Fetching sponsor data for memberCode:',
      memberCode
    );
    ...
    if (!inviterRootSponsorId) {
      console.log(
        '[INVITES-SEND] ⚠️ Inviter missing rootSponsorId, setting to self'
      );
    ...
    try {
      const qrResponse = await fetch(
        `${process.env.NEXT_PUBLIC_BASE_URL || 'http://localhost:3000'}/api/invites/generate-qr`,
        {
          method: 'POST',
          headers: {
      ...
      if (qrResponse.ok) {
        ...
        console.log(
          '✅ [INVITES-SEND] QR code generated successfully:',
          qrData.qrCodeUrl
        );
      } else {
        console.log(
          '⚠️ [INVITES-SEND] QR code generation failed, but invite was created'
        );
      }
    } catch (qrError) {
    ...
    console.log(
      '[INVITES-SEND] 🔍 Checking for cross-connection TU prospect...'
    );
    ...
    try {
      ...
      if (!inviteeQuery.empty) {
        ...
        if (
          inviteeData?.sponsorId === memberData?.sponsorId &&
          inviteeData?.sponsorId !== '0000000000'
        ) {
          console.log('🎯 [INVITES-SEND] CROSS-CONNECTION DETECTED!');
          console.log(
            `   Inviter: ${memberCode} (sponsor: ${memberData?.sponsorId})`
          );
          console.log(
            `   Invitee: ${inviteeMemberCode} (sponsor: ${inviteeData?.sponsorId})`
          );
          ...
          const prospectResult = await createTUProspect(
            memberCode,
            inviteeMemberCode
          );
          ...
          if (prospectResult.success) {
            console.log(
              `✅ [INVITES-SEND] TU prospect created: ${prospectResult.unitId}`
            );
            console.log(`   Status: ${prospectResult.status}`);
            console.log(
              `   Members: [${prospectResult.members?.join(', ') || 'none'}]`
            );
            ...
            console.log(
              `❌ [INVITES-SEND] Failed to create TU prospect: ${prospectResult.error}`
            );
          }
        } else {
          console.log(
            '[INVITES-SEND] No cross-connection (different sponsors or admin)'
          );
        }
      } else {
        console.log(
          '[INVITES-SEND] Invitee not yet registered, no cross-connection check'
        );
      }
    } catch (crossError: any) {
      console.error(
        '[INVITES-SEND] Error in cross-connection detection:',
        crossError
      );
    ...
    if (!nfSnapshot.empty) {
      ...
      console.log(
        '[INVITES-SEND] ⚡ MATCH FOUND in notFoundRegistry:',
        nfDoc.id
      );
      ...
      console.log(
        '[INVITES-SEND] ✅ Updated notFoundRegistry status to "invited"'
      );
    }
