...
export async function POST(request: NextRequest) {
  try {
    ...
    const { inviteId, inviteeName, inviteePhone, inviterName, inviterCode } = body;
    
    console.log('[QR-GENERATE] Request:', { inviteId, inviteeName, inviteePhone, inviterName, inviterCode });
    ...
    console.log('[QR-GENERATE] QR code buffer generated, size:', qrCodeBuffer.length);

    // Use base64 for now (Firebase Storage needs real API key)
    console.log('[QR-GENERATE] Using base64 QR code (Firebase Storage disabled)');
