...
import { safeGet, safeUpperCase, safeArray, safeObject, logError } from "@/utils/errorHandling";
...
export default function AdminDashboard() {
  ...
  const loadDashboardData = async () => {
    try {
      ...
      if (statsData.ok) {
        setStats(safeObject(statsData.stats, {
          totalMembers: 0,
          newToday: 0,
          pendingMembers: 0,
          registeredMembers: 0
  ...
  const confirmClearAll = async () => {
    ...
    try {
      ...
      if (data.ok) {
        ...
        setSuccessMessage(`Successfully cleared ${data.deletedMembers} members and ${data.deletedInvites} invites (${data.totalDeleted} total items)`);
  ...
  const confirmDeleteMember = async () => {
    ...
    try {
      ...
      const response = await fetch(`/api/admin/delete-user?memberCode=${memberCode}`, {
        method: 'DELETE'
      });
      const data = await response.json();
  ...
  const handleSendInvitation = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (data.ok) {
        setSuccessMessage(`Invitation sent successfully to ${inviteForm.name}!`);
        setShowSuccessModal(true);
  ...
  const handleSetActive = async (invite) => {
    try {
      ...
      if (data.ok) {
        setSuccessMessage(`Invite status updated to ${invite.status === 'active' ? 'sent' : 'active'}`);
        setShowSuccessModal(true);
  ...
  const renderMembersContent = () => (
      ...
      <h2 className="text-2xl font-bold text-slate-800 mb-6">Member Management</h2>
      <div className="mb-4">
            <input
              type="text"
        ...
        <table className="min-w-full divide-y divide-slate-200">
          <thead className="bg-slate-50">
            <tr>
              ...
              <th className="px-6 py-3 text-left text-xs font-medium text-slate-500 uppercase tracking-wider">Sponsor</th>
              <th className="px-6 py-3 text-left text-xs font-medium text-slate-500 uppercase tracking-wider">Member Code</th>
          ...
          <tbody className="bg-white divide-y divide-slate-200">
            {filteredMembers.map((member) => (
              <tr key={member.memberCode} className="hover:bg-slate-50">
                ...
                <td className="px-6 py-4 whitespace-nowrap">
                  <div className="text-sm text-slate-900">{member.sponsorName || 'N/A'}</div>
                    </td>
                <td className="px-6 py-4 whitespace-nowrap">
                  <div className="text-sm text-slate-900 font-mono">{member.memberCode}</div>
                    </td>
                <td className="px-6 py-4 whitespace-nowrap">
          ...
          <div className="mt-8">
            <h3 className="text-xl font-bold text-slate-800 mb-4">Not Found Registry</h3>
            <p className="text-sm text-slate-600 mb-4">
              Phone numbers captured from users who weren&apos;t found in the system (4-step flow)
            </p>
            <div className="overflow-x-auto">
              <table className="min-w-full divide-y divide-slate-200">
                <thead className="bg-slate-50">
                  ...
                  {notFoundRegistry && notFoundRegistry.length > 0 ? (
                    notFoundRegistry.map((entry) => (
                      <tr key={entry.id} className="hover:bg-slate-50">
                        ...
                        <td className="px-6 py-4 whitespace-nowrap">
                          <div className="text-sm text-slate-500">{entry.sponsor || 'no spon'}</div>
                        </td>
                        <td className="px-6 py-4 whitespace-nowrap">
                          <div className="text-sm text-slate-900 font-mono">{entry.memberCode}</div>
                        </td>
                        <td className="px-6 py-4 whitespace-nowrap">
  ...
  const renderAnalyticsContent = () => (
    <div className="bg-white rounded-lg p-6 shadow-sm border border-slate-200">
      ...
      <div className="text-center py-12">
        ...
        <p className="text-slate-500">Advanced analytics and reporting features coming soon.</p>
      </div>
    </div>
  );
  ...
  const renderVoicePrintsContent = () => (
    <div className="bg-white rounded-lg p-6 shadow-sm border border-slate-200">
      <h2 className="text-2xl font-bold text-slate-800 mb-6">Voice Prints Management</h2>
      <div className="text-center py-12">
        <div className="text-6xl mb-4">🎵</div>
        <h3 className="text-xl font-semibold text-slate-700 mb-2">Voice Prints</h3>
        <p className="text-slate-500">Voice print management and verification tools coming soon.</p>
      </div>
    </div>
  );
  ...
  const renderSecurityContent = () => (
    <div className="bg-white rounded-lg p-6 shadow-sm border border-slate-200">
      <h2 className="text-2xl font-bold text-slate-800 mb-6">Security Center</h2>
      <div className="text-center py-12">
        <div className="text-6xl mb-4">🔒</div>
        <h3 className="text-xl font-semibold text-slate-700 mb-2">Security Dashboard</h3>
        <p className="text-slate-500">Security monitoring and threat detection tools coming soon.</p>
      </div>
    </div>
  );
  ...
  const renderSettingsContent = () => (
    <div className="bg-white rounded-lg p-6 shadow-sm border border-slate-200">
      ...
      <div className="text-center py-12">
        ...
        <h3 className="text-xl font-semibold text-slate-700 mb-2">System Configuration</h3>
        <p className="text-slate-500">System settings and configuration options coming soon.</p>
          </div>
        </div>
  );
  ...
  const renderInviteManagementContent = () => (
    <>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        ...
        <div className="bg-white rounded-lg p-6 shadow-sm border border-slate-200">
          <h2 className="text-2xl font-bold text-slate-800 mb-6">Send New Invitation</h2>
          ...
          <div className="bg-blue-50 rounded-lg p-6 border border-blue-200">
            <form onSubmit={handleSendInvitation} className="space-y-4">
              ...
              <div className="bg-yellow-50 border border-yellow-200 rounded-lg p-4">
                <h4 className="font-semibold text-yellow-800 mb-2">Message Preview:</h4>
                <div className="text-sm text-yellow-700 bg-white p-3 rounded border">
                  <p><strong>Hello {inviteForm.name || '[Name]'}!</strong></p>
                  ...
                  <p>Talk soon,<br/><strong>ADMIN</strong></p>
                </div>
              </div>
        ...
        <div className="bg-white rounded-lg p-6 shadow-sm border border-slate-200">
          <h3 className="text-lg font-semibold text-slate-700 mb-4">Invitation Preview</h3>
              ...
              <div className="text-sm text-slate-700 leading-relaxed">
                ...
                <div className="border-t border-yellow-300 pt-3 mt-3">
                  <p className="text-sm text-slate-600 italic">
                    Hi {inviteForm.name || '[first name]'}, I need you! Will you verify I am human! It&apos;s part of a network I am building. It maybe something you will be interested in. Why? No bots, no scammers, just the people I love... I love you!! Thank you! ADMIN
                  </p>
                </div>
              </div>
              </div>
      ...
      <div className="bg-white rounded-lg p-6 shadow-sm border border-slate-200 mt-6">
        <h3 className="text-lg font-semibold text-slate-700 mb-4">Sent Invites</h3>
        <div className="bg-gray-50 rounded-lg p-4 border border-gray-200">
          {inviteHistory.length === 0 ? (
            <div className="text-center py-8">
              <div className="text-4xl mb-4">📋</div>
              <p className="text-slate-600">No invitations sent yet.</p>
              <p className="text-sm text-slate-500 mt-2">Invitation history will appear here.</p>
            </div>
          ) : (
                    ...
                    <tr key={index} className="border-b border-gray-200 hover:bg-gray-50">
                      ...
                      <td className="py-2 px-3">
                        <div className="flex space-x-2">
                          ...
                          >
                            {(invite.status || 'pending') === 'active' ? 'Active' : 'Set Active'}
                          </button>
                <button
  ...
  const renderLogsContent = () => (
    <div className="bg-white rounded-lg p-6 shadow-sm border border-slate-200">
          ...
          <table className="min-w-full divide-y divide-slate-200">
            <thead className="bg-slate-50">
              <tr>
                ...
                <th className="px-6 py-3 text-left text-xs font-medium text-slate-500 uppercase tracking-wider">Member Code</th>
                <th className="px-6 py-3 text-left text-xs font-medium text-slate-500 uppercase tracking-wider">Archived At</th>
                <th className="px-6 py-3 text-left text-xs font-medium text-slate-500 uppercase tracking-wider">Status</th>
              </tr>
            </thead>
            <tbody className="bg-white divide-y divide-slate-200">
              {nfArchive && nfArchive.length > 0 ? (
                nfArchive.map((entry) => (
                  <tr key={entry.id} className="hover:bg-slate-50">
                    ...
                    <td className="px-6 py-4 whitespace-nowrap">
                      <div className="text-sm text-slate-500">{entry.phone}</div>
                    </td>
                    <td className="px-6 py-4 whitespace-nowrap">
                      <div className="text-sm text-slate-900 font-mono">{entry.memberCode}</div>
                    </td>
      ...
      <div className="text-center py-12">
        <div className="text-6xl mb-4">📋</div>
        <h3 className="text-xl font-semibold text-slate-700 mb-2">System Logs</h3>
        <p className="text-slate-500">Additional system logs and audit trails coming soon.</p>
      </div>
    </div>
  );
  ...
  const renderBackupContent = () => (
    <div className="bg-white rounded-lg p-6 shadow-sm border border-slate-200">
      <h2 className="text-2xl font-bold text-slate-800 mb-6">Backup & Restore</h2>
      <div className="text-center py-12">
        <div className="text-6xl mb-4">💾</div>
        <h3 className="text-xl font-semibold text-slate-700 mb-2">Backup Management</h3>
        <p className="text-slate-500">Data backup and restore functionality coming soon.</p>
      </div>
              </div>
  );
