...
export default function VaultsPage() {
  ...
  const loadMemberData = async (code: string) => {
    try {
      ...
      // Load trust bonds
      const trustBondsResponse = await fetch(`/api/trust-bonds/list?memberCode=${code}`);
      if (trustBondsResponse.ok) {
        const trustBondsData = await trustBondsResponse.json();
        if (trustBondsData.ok) {
          setTrustBonds(trustBondsData.trustBonds || []);
        }
      }

      // Load trust units
      const trustUnitsResponse = await fetch(`/api/trust-units/list?memberCode=${code}`);
      if (trustUnitsResponse.ok) {
        const trustUnitsData = await trustUnitsResponse.json();
        if (trustUnitsData.ok) {
          setTrustUnits(trustUnitsData.trustUnits || []);
        }
      }
    } catch (err) {
        ...
        <main className='flex-1'>
          ...
          <div className='p-6'>
            <div className='bg-white rounded-xl border border-slate-200 p-6'>
              <h2 className='text-xl font-semibold text-slate-800 mb-4'>Vaults Content</h2>
              <p className='text-slate-600'>This is your Vaults page content.</p>
            </div>
          </div>
        </main>
      </div>
